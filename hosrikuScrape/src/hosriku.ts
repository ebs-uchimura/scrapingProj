...
ipcMain.on('scrapeurl', async (event: any, arg: any) => {
  try {
    ...
    // pref padded
    logger.debug(`scrapeurl: ${myConst.HOSRIKU_BASE}recruit?pref=${arg}`);
    // initialize scraper
    await puppScraper.init(false);
    // goto top
    await puppScraper.doGo(`${myConst.HOSRIKU_BASE}recruit?pref=${arg}`);
    // wait for datalist
    await puppScraper.doWaitFor(myProperties.WAIT_SECOND);
    // url exists
    if (!await puppScraper.doCheckSelector(mySelector.hosRikuTotalSelector)) {
    ...
    const tmpHosparatotal: any = await puppScraper.doMultiEval(
      mySelector.hosRikuTotalSelector,
      'innerHTML'
    );
    // wait for datalist
    await puppScraper.doWaitFor(myProperties.WAIT_SECOND);
    ...
    for (let areaNum of areaNumberArray) {
      try {
        ...
        // area url
        const areaUrl: string = `${myConst.HOSRIKU_BASE}recruit?pref=${arg}${fixedQuery}`;
  ...
  try {
    ...
    for (let nm of totalNumber) {
      try {
        ...
        // shopname
        const shopname: string = await doScrape(mySelector.hosRikuMainShopnameSelector);
        ...
        // holiday
        const holiday: string = await doScrape(mySelector.hosRikuHolidaySelector);
        await puppScraper.doWaitFor(myProperties.WAIT_MILLSECOND);
        const checkedHoliday: string = checkEvaluation(holiday);
        myShopObj['holiday'] = checkedHoliday;
        // address1
        const address1: string = await doScrape(mySelector.hosRikuAddressSelector);
        await puppScraper.doWaitFor(myProperties.WAIT_MILLSECOND);
        const checkedAddress1: string = checkEvaluation(address1);
        myShopObj['address'] = checkedAddress1;
        // businesstime
        const businesstime: string = await doScrape(mySelector.hosRikuBusinesstimeSelector);
        await puppScraper.doWaitFor(myProperties.WAIT_MILLSECOND);
        const checkedBusinesstime: string = checkEvaluation(businesstime);
        myShopObj['businesstime'] = checkedBusinesstime;
        // shopphone
        const shopphone: string = await doScrape(mySelector.hosRikuTelephoneSelector);
        await puppScraper.doWaitFor(myProperties.WAIT_MILLSECOND);
