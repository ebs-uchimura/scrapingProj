...
export class Scrape {
  ...
  // attributes
  doMultiAttributes(selector: string): Promise<string[]> {
    return new Promise(async (resolve, reject) => {
      try {
        //Scrape.logger.debug('scrape: doMultiEval mode.');
        // data set
        let datas: string[] = [];
        // target list
        const styleNumbers: any = await Scrape.page.$$(selector);
        // result
        const result: boolean = await Scrape.page.$(selector).then((res: any) => !!res);
        ...
        if (result) {
          ...
          for (const styleNumber of styleNumbers) {
            datas.push(await Scrape.page.evaluate((el: any) => el.getAttribute("data-url"), styleNumber));
          }
          // resolved
          resolve(datas);
        }
