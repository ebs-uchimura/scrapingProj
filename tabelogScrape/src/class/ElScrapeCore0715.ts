...
export class Scrape {
  ...
  // eval
  doMultiEval(selector: string, property: string): Promise<string | string[]> {
    return new Promise(async (resolve, reject) => {
      try {
  ...
  doClose(): Promise<void> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // resolved
        await Scrape.closeBrowser(Scrape.page, Scrape.browser);
        Scrape.logger.debug('Browser closed successfully.');
        // resolve
        resolve();
      }
    });
  ...
  static closeBrowser = async (
    ...
    await browser.disconnect(); // 接続を解除
    await browser.close(); // ブラウザを閉じる
    browser.process()?.kill(9); // プロセスを強制終了
  };
}
